...
Copyright (c) 2008-2014 by Barry Barnreiter (barry_b@users.sourceforge.net)
...
Place the contents of the OpenOPC distribution zip file into the directory
where you want the files to be permanently installed.   Then run the included
INSTALL.bat file.

If you run INSTALL.bat with no arguments you will receive the default
configuration settings.   However you can configure the service further
during installation using the following syntax:

INSTALL.bat [ip address] [inactivity time] [max clients]

Example:
INSTALL.bat 10.0.0.57 30

The above example configures the OpenOPC Gateway service to listen only
on the 10.0.0.57 ethernet interface and to automatically close sessions
that have been inactive for more than 30 minutes.
